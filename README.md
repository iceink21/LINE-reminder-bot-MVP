...
3. `finalizeResult()` ใน `gemini.js` ตัดสินตามลำดับ: **(1)** `recurring === true` → ไม่มีกำหนดส่งเลย · **(2)** `relative_weekday` ที่แปลงได้ → ใช้วันที่ที่คำนวณเอง **ทับ** `deadline_iso` ที่โมเดลส่งมาด้วย · **(3)** นอกนั้น → `deadline_iso` ตามเดิม
...
> `อาทิตย์` กำกวม: แปลได้ทั้ง **วันอาทิตย์** และ **สัปดาห์** — `อาทิตย์หน้า` แบบไม่มี `วัน` นำหน้า คนไทยหมายถึง "สัปดาห์หน้า" ไม่ใช่ "วันอาทิตย์หน้า" ถ้าปล่อยให้ลงช่อง `relative_weekday` งานจะถูกเตือนเร็วกว่าที่ตั้งใจได้ถึง 6 วัน — แก้ที่ **prompt** (`SYSTEM_RULES` ใน `gemini.js`) ไม่ใช่ที่ `resolveWeekday`: บอกโมเดลให้ตีความ `อาทิตย์หน้า`/`อาทิตย์นี้` (ไม่มี `วัน`) เป็นสัปดาห์แล้วใช้ `deadline_iso` ตามปกติ ส่วน `วันอาทิตย์หน้า`/`วันอาทิตย์นี้` ยังหมายถึงวันอาทิตย์และใช้ช่องสัญลักษณ์เหมือนเดิม ฝั่ง `resolveWeekday` ยังคงแปลง `อาทิตย์หน้า` เป็นวันอาทิตย์ครั้งถัดไป (มี test ปักไว้) — เป็น fallback ที่ตั้งใจ เผื่อโมเดลยังเผลอกรอกช่องนั้นมา
